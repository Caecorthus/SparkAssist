...
2. Treat the local public Interface as stable.

   The stable Interface includes:
...
9. Tests should cross the same Interface as callers.
...
10. Treat generated output as non-source.
...
`EventSoundVolumeRules` is the stable public lookup and volume-rule Interface.
...
keep those identifier families readable and explicit.
...
```bash
JAVA_HOME=$(/usr/libexec/java_home -v 21) ./gradlew test --no-daemon --no-watch-fs --console=plain
git diff --check
```
