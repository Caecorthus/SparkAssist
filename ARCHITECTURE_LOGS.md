...
- Restored JUnit 5 and focused config, toggle, catalog, positive-routing, and
  negative-isolation tests after the earlier test-removal commit left two test
  sources unable to compile.
- Added duplicate identifier ownership rejection to `EventSoundCatalog`.
- Declared existing SparkTraits identifier support in `fabric.mod.json` suggests.
- Added `verifyArchitecture` to enforce client-only metadata and the required
